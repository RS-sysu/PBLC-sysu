% the result of ann_pbl is rescaled by a constant factor
...
input2 = [outdir, '\ann\lamda2_0.0\par_', num2str(nT), '_ann_pbl_a.csv'];
par = dlmread(input2);

acc = [acc par(:, 2:end)];

for k = 1:10
	input3 = [outdir, '\ann\lamda2_0.0\pre_', num2str(nT), '_', num2str(k), '_ann_pbl_a.csv'];
	pre_all = dlmread(input3);

	pre_all = pre_all / max(pre_all(:));			% rescale the probability to make maximum value equal 1
