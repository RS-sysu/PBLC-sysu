...
for s_id in range(0, 3):
	...
	# for k in range(0, 2):
		...
		for n in range(0, nk):
			...
			ypre = ypre + sess.run(tf.sigmoid(prediction), {xs: x_test})
		...
		output1 = r'C:\backup_d\exp\PBL_exp\logit\output\pre_' + str(nT) + '_' + str(k + 1) + '_glm_pa.csv'
		np.savetxt(output1, ypre, delimiter=',', fmt='%f')
		
		del ypre, data_train, x_data, y_data
		gc.collect()
		
	output2 = r'C:\backup_d\exp\PBL_exp\logit\output\par_' + str(nT) + '_glm_pa.csv'
