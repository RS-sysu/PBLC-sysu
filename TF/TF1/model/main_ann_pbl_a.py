...
for s_id in range(2, 3):
	...
	# for k in range(0, 5):
		...
		for n in range(0, nk):
			...
			# c = np.mean(sess.run(prediction, {xs: x1, dropout_keep_prob: keep_prob_val}))
			# c = sess.run(tf.reduce_mean(tf.sigmoid(sess.run(prediction, {xs: x1, dropout_keep_prob: keep_prob_val}))))
			c = np.mean(sess.run(tf.sigmoid(prediction), {xs: x1, dropout_keep_prob: keep_prob_val}))
			
			# ypre = sess.run(prediction, {xs: x_test, dropout_keep_prob: keep_prob_val})	
			# ypre = sess.run(tf.sigmoid(sess.run(prediction, {xs: x_test, dropout_keep_prob: keep_prob_val})))
			ypre = sess.run(tf.sigmoid(prediction), {xs: x_test, dropout_keep_prob: keep_prob_val})
		...
		output1 = r'C:\backup_d\exp\PBL_exp\logit\output\pre_' + str(nT) + '_' + str(k + 1) + '_ann_pbl_a.csv'
		np.savetxt(output1, ypre1, delimiter=',', fmt='%f')
		
		output2 = r'C:\backup_d\exp\PBL_exp\logit\output\pre_' + str(nT) + '_' + str(k + 1) + '_ann_pul_b.csv'
		np.savetxt(output2, ypre2, delimiter=',', fmt='%f')
		
		del ypre1, ypre2, data_train, x_data, y_data
		gc.collect()
	
	output3 = r'C:\backup_d\exp\PBL_exp\logit\output\par_' + str(nT) + '_ann_pbl_a.csv'
