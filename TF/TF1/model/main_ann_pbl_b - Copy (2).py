...
def add_layer(inputs, in_size, out_size, is_norm, dropout_keep_prob, is_dropout, activation_function = None):
	...
	if is_dropout == 1:
		outputs = tf.nn.lrn(outputs, depth_radius = 5, bias = 2.0, alpha = 1e-3, beta = 0.75)
...
lamda2 = 0.0			# regularization parameter
lamda3 = 0.0			# regularization parameter
num_iter = 2000         # 5000
eval_every = 50
tra_rate = 0.75
...
layer1, Weights_1, biases_1 = add_layer(xs, n_feat, 10, 1, dropout_keep_prob, 1, activation_function = tf.nn.relu)
layer2, Weights_2, biases_2 = add_layer(layer1, 10, 5, 1, dropout_keep_prob, 1, activation_function = tf.nn.relu)		# tf.tanh
prediction, Weights_3, biases_3 = add_layer(layer2, 5, 1, 0, dropout_keep_prob, 0, activation_function = tf.sigmoid)
...
# prediction2 = prediction / (prediction + 2.5)		# PBL: true value of c(0.2857) is provided
# prediction2 = prediction * c								# PUL

l2_reg = lamda1 * (tf.nn.l2_loss(Weights_1) + tf.nn.l2_loss(Weights_2) + tf.nn.l2_loss(Weights_3))
c2_reg = lamda2 * tf.nn.l2_loss(tf.subtract(tf.reduce_max(prediction), max_pr_y1))
y1_reg = lamda3 * tf.nn.l2_loss(tf.subtract(1.0 / (1.0 + r_bac * tf.reduce_sum(prediction * (1 - ys)) / tf.reduce_sum(1 - ys)), c))
...
loss = tf.add(tf.add(tf.add(cross_ent, l2_reg), c2_reg), y1_reg)
...
for s_id in range(1, 2):
	...
	# for k in range(0, 2):
		...
		for n in range(0, nk):
			...
			ypre = ypre + sess.run(prediction, {xs: x_test, dropout_keep_prob: keep_prob_val})		
		...
		output1 = r'C:\backup_d\exp\PBL_exp\logit\output\pre_' + str(nT) + '_' + str(k + 1) + '_ann_pbl_b.csv'
		np.savetxt(output1, ypre, delimiter=',', fmt='%f')
		
		del ypre, data_train, x_data, y_data
		gc.collect()
	
	output2 = r'C:\backup_d\exp\PBL_exp\logit\output\par_' + str(nT) + '_ann_pbl_b.csv'
