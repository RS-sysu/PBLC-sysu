...
# for k in range(1):
	...
	x1 = x_train[np.any(y_train == 1, axis = 1),:]			# use boolean indexing to pull out rows or columns meeting some criterion
	c = np.mean(sess.run(prediction, {xs: x1}))

	ypre = sess.run(prediction, {xs: x_test1})	
	ypre1 = (1.0 - c) / c * ypre / (1.0 - ypre)
	
	ypre = sess.run(prediction, {xs: x_test2})
	ypre2 = (1.0 - c) / c * ypre / (1.0 - ypre)
	...
	output3 = r'C:\backup_d\exp\PBL_exp\logit\output\pul_pre_200_' + str(k + 1) + '_all.csv'
	output4 = r'C:\backup_d\exp\PBL_exp\logit\output\pul_pre_200_' + str(k + 1) + '_PU.csv'
	np.savetxt(output3, ypre3, delimiter=',', fmt='%f')
	np.savetxt(output4, ypre4, delimiter=',', fmt='%f')
	
output5 = r'C:\backup_d\exp\PBL_exp\logit\output\glm_par.csv'
np.savetxt(output5, par, delimiter=',', fmt='%f')
